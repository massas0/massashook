		...
		if (pEntity)
		{
			...
			if (g_Options.Skinchanger.Enabled)
			{
				...
				if (weapon)
				{
					...
					if (g_Options.Skinchanger.W[weapon].customstatamount)
					{
						*pWeapon->FallbackStatTrak() = g_Options.Skinchanger.W[weapon].customstatamount;
					}

					if (g_Options.Skinchanger.W[weapon].Name)
					{
						*pWeapon->GetCustomName() = g_Options.Skinchanger.W[weapon].Name;
					}
					...
					if (pEntity->GetClientClass()->m_ClassID)
					{
						if (pEntity->GetClientClass()->m_ClassID == (int)ClassID::CKnife)
						{
							...
							*pWeapon->m_AttributeManager()->m_Item()->ItemDefinitionIndex() = GetKnifeDefinitionIndex(g_Options.Skinchanger.knifemodel - 1);
							...
							if (g_Options.Skinchanger.W[GetKnifeDefinitionIndex(g_Options.Skinchanger.knifemodel - 1)].customstat)
							{
								*pWeapon->FallbackStatTrak() = g_Options.Skinchanger.W[GetKnifeDefinitionIndex(g_Options.Skinchanger.knifemodel - 1)].customstatamount;
							}

							if (g_Options.Skinchanger.W[GetKnifeDefinitionIndex(g_Options.Skinchanger.knifemodel - 1)].Name)
							{
								*pWeapon->GetCustomName() = g_Options.Skinchanger.W[GetKnifeDefinitionIndex(g_Options.Skinchanger.knifemodel - 1)].Name;
							}
