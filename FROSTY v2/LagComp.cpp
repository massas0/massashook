...
void BackTrack::UpdateRecord(int i)
{
	...
	if (pEntity && pEntity->IsAlive() && !pEntity->IsDormant())
	{
		...
		records[i].lby = pEntity->GetLowerBodyYaw();;
		records[i].headPosition = GetHitboxPosition(pEntity, Head);
	}
	else
	{
...
{
	if (g_Options.Legitbot.backtrack || g_Options.Ragebot.PosAdjust)
	{
		...
		if (bestTargetIndex != -1)
		{
			...
			else if (g_Options.Ragebot.PosAdjust)
			{
				for (int t = 0; t < 12; ++t)
				{
					float tempFOVDistance = distance_point_to_line(headPositions[bestTargetIndex][t].hitboxPos, pLocal->GetEyePosition(), ViewDir);
